...
- **Active feature step:** applying the stateful session's changing tier plans to active media branches is **built and verified on hardware** (`vms_grid --sweep`, whole-grid NULL→rebuild on each plan change); the governor's tier decisions **select real per-camera RTSP main/sub streams** (`--govern --camera "main;sub"`, plumbing verified locally; live-camera decode pending a credentialed run); and the **honest per-tile state model** (live/degraded/paused-offscreen/paused-capacity) is built, unit-tested, and surfaced live in the harness. Remaining native-verifiable work is thin; the next steps are gated or blocked: the P3-14 Qt/QML visual state UI, a seamless (non-reloading) live apply, and (blocked on hardware) live-camera RTSP confirmation and low-end i5 calibration.
...
1. **Delivered-runtime lane (native, N0):** the active build track. P3-03 is approved and increment 4b is in progress; applying stateful tier changes to active media branches and selecting real per-camera RTSP main/sub streams by tier are now built (RTSP plumbing verified locally; live-camera decode pending a credentialed run), so the next unblocked code steps are honest Qt/QML per-tile state and a seamless (non-reloading) live apply, while corrected dev-box and low-end measurements remain independent calibration tasks. See the Native Client Track section above.
