...
int main() {
    ...
    // 1) Open + migrate the canonical schema.
    {
        ...
        check(s.schemaVersion() == 7, "schema at version 7");
    ...
    // 3) Durability + idempotency + atomicity on reopen.
    {
        ...
        check(s.schemaVersion() == 7, "schema still at version 7 after reopen");
        ...
        check(count(s, "SELECT COUNT(*) FROM schema_migrations;") == 7,
              "exactly seven migrations recorded (not duplicated)");
