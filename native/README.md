...
deferred coverage exemption. **Next up:** a scope decision for the premises/site model and remaining
P3-15 floor-plan/FOV work (the dependency for P3-18); in parallel, a seamless non-reloading live apply
remains a media optimization and live-camera RTSP confirmation plus low-end i5 calibration remain
hardware-blocked.
...
| 26 (P1-13/A0) | **External control API** — the transport in front of the increment-25 envelope, so an external AI agent (or any client) drives the software with the **same authority model as a human operator**. `CommandServer` (`src/workspace/`, Qt HttpServer): **loopback-only** (binds 127.0.0.1, never public), **off by default** (`--api-port N`), **bearer-token** auth on every route (a random per-session token printed once; `--api-token` for scripted runs). `GET /v1/commands` = the machine-readable catalog (agent discovery); `POST /v1/invoke` `{"command","args","confirm"}` flows through the SAME registry gate — validation, capability, dangerous confirm, audit — with a deterministic status mapping (ok→200, unknown-command→404, missing/bad-param→400, capability-denied→403, **needs-confirm→409**, failed→500) and the same `{outcome,message}` body shape everywhere. Scope: [`command-api-P1-13-proposal.md`](command-api-P1-13-proposal.md). `--api-selftest` | ✅ built & verified (dev box): `--api-selftest` green — **16 checks** (401 without/with-wrong token, authenticated catalog, a real invoke moves the running workspace, typed JSON args bind, 404/400/409/500 mapping, malformed body 400, honest JSON 404 route, shared audit trail with refusals). **Proven end-to-end against the running app:** an external PowerShell client discovered the 11-command catalog, resized the live wall (`layout 9`) and focused a tile over HTTP, was **blocked 409** on an unconfirmed `device.remove` and **401** without the token — every attempt in the app's audit trail. All 10 CTest suites + all 5 workspace selftests green; offscreen smoke clean. **Remaining (rest of P1-13):** multi-user/scoped tokens, TLS, rate limits, result/event streaming, non-loopback exposure (security-reviewed), the AI-agent connector (a client of this API), and the UI-coverage check |
...
.\build\vms_workspace.exe --api-port 18790
