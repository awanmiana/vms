...
# Switch to the "Devices" tab. Onboard a direct IP camera, or pick a recorder
# kind (NVR/DVR/Hybrid) and give a channel count + RTSP URL templates with a
# {ch} placeholder to onboard all channels at once (inc 15). Acknowledge / put
# in maintenance / remove a seeded one. Headless verification (no display):
.\build\vms_workspace.exe --devices-selftest            # 17-check controller logic
