...
| 29 (P1-13/P1-09) | **Command coverage — no UI action bypasses the envelope.** Closes P1-13's enforcement half. `CommandController::invoke(id, argsMap, confirm)` gives QML a **structured** path through the same gate (typed args: names with spaces, URLs, credentials — no text tokenization); 12 new commands cover the rest of the discrete surface (`device.onboard/onboardRecorder/onboardDiscovered/discover/rescan/acknowledge/maintenance`, `channel.rename/disable/remove` (dangerous)`/move`, `workspace.place`); and **every one of the 27 direct controller call sites in the shipped QML is rerouted** — with *no* direct fallback, so a bypass cannot hide. The gate itself is a pure `CoverageCheck` that scans the **shipped** QML out of the Qt resource (what actually ships, not a stale source path), classifies each call against a declared 39-mutator table, and exits **non-zero** on a violation (`--coverage-check`, CI-gateable). Exemptions are declared, categorized and **printed** — pure reads, continuous viewport/scrub frames (the *committed* gesture is `workspace.place`), and the Playback transport named as a deferred slice rather than hidden. **Security fix found by this work (P1-09):** an end-to-end run showed `password=s3cret` landing in the durable audit; `ParamSpec::secret` + a name-based backstop now redact secrets to `***` in every audit row, log line, and the catalog, while the handler still receives the real credential. Scope: [`command-coverage-P1-13-proposal.md`](command-coverage-P1-13-proposal.md) | ✅ built & verified (dev box): `--coverage-check` **0 violations** across 2 shipped QML files / 39 declared mutators / 18 remaining sites (all declared exemptions, each printed with its reason); `--coverage-selftest` **10 checks** incl. the negative test (an injected `devicesCtrl.removeDevice(...)` **is** detected at the exact line), comment-not-a-call, and identifier-boundary; `command_selfcheck` now **37 checks** (adds: handler still gets the real secret, audit shows `password=***`, non-secrets recorded in full, catalog marks `"secret":true`, unknown-command backstop). **Leak closed end-to-end:** after an API onboard, a raw byte-scan of the audit DB and console log finds no plaintext secret, and the device still onboarded. All 11 CTest suites + 10 workspace gates green; smoke clean. **Remaining:** voice leg (P1-12), Playback-transport commands, scoped tokens/TLS/streaming (P1-13 transport), roles (P1-02/03) |
