...
| 5a (P0-04) | `vms_persist` — embedded SQLite store behind the C0-03 adapter contract (declared tables, atomic writes + RAII rollback, versioned forward migrations, typed errors, online backup), via the Windows-bundled **winsqlite3** (no download/vendoring; Qt-free, swappable backend). `vms_dbtest`/CTest | ✅ built & verified (dev box): `persist_selfcheck` green — 19 checks: migrate/idempotent, durability across reopen, FK constraint, atomic rollback, backup round-trip. P0-04 scope approved 2026-07-26 (`persistence-P0-04-proposal.md`). Next: 5b entities + export/import, 5c wire the workspace to persist/restore its layout |
