...
scaffold honestly marked "awaiting recorded footage (Phase 4)". The Playback range/timeline/transport
is now wired to real local recordings, and native increment **30** routes its discrete controls (plus
instant-replay transport) through the shared command envelope/API with committed seeks audited and no
deferred coverage exemption. Later increments completed P3-15 and the standalone P3-18 premises panel.
Increment 41 now replaces only changed media branches during same-layout governor applies while the
last complete frame remains visible; unchanged decoders survive. Live-camera RTSP confirmation with
valid credentials and real low-end i5 calibration remain hardware-blocked.

Increment 47 makes RTSP media transport explicit instead of silently forcing TCP. Both `vms_spike`
and `vms_grid` accept `--rtsp-transport auto|tcp|udp|multicast`; `auto` permits the standardized
UDP-unicast → UDP-multicast → interleaved-TCP negotiation, while deployments can pin a compatible
NVR/firewall path to TCP. RTCP remains enabled and UDP fallback/TCP connection timeouts are bounded.
The pure `vms_media_policy` self-check verifies the mapping. The previously intermittent aggregate
ONVIF CTest was a missing Qt runtime `PATH` (`0xc0000135`), not a protocol parser defect: CTest now
sets the target Qt runtime directory and a 15-second timeout, passes 50/50 clean-shell stress runs,
and the aggregate native suite passes 12/12. See `connectivity-protocols-P2-14-proposal.md` for the
standards/discovery matrix and the boundary between installed runtime elements and product support.
...
| 23 (P3-05) | **Instant playback from live view** — the "instant replay" verb. `InstantReplayController` (`src/workspace/`, Qt + `vms_persist`) OWNS a `PlaybackController` (no duplication) pinned to a look-back window: `replay(seconds)` computes the camera's **live edge** (latest recorded moment), opens `[edge − seconds, edge]`, positions the playhead at the window start, and auto-plays through a dedicated `PlaybackPipeline` → a third `instantVideoOut` `VideoItem`. Honest per P0-03: a camera with no local recording opens the overlay **unavailable** (never presents requested time as recorded); a look-back longer than the footage **clamps** to the earliest recorded moment and reports the *actual* look-back. UI: an "Instant replay · 10s/30s/60s" control on the Live info panel + a layered replay overlay over the (still-running) live wall with a compact transport and a "⟵ Live" return. Routed through `Q_INVOKABLE` `replay`/`returnToLive` so it can later be an A0 command. `--instant-selftest` + offscreen `--smoke-ms --rec-db`. Scope: [`instant-playback-P3-05-proposal.md`](instant-playback-P3-05-proposal.md) | ✅ built & verified (dev box): `--instant-selftest` green — **22 checks** (starts inactive; live edge = latest recorded; replay(30) opens a window ending at the edge / starting 30s before / playhead ON footage / auto-plays / a file backs the playhead; look-back 600 clamps to earliest + reports 90s honestly; return-to-live closes + pauses; an unrecorded camera is honestly unavailable with no fabricated spans). Offscreen smoke against a real `vms_record` DB: the replay overlay binds with **no QML errors/warnings** and the instant decode pulled **89 real frames** (look-back honestly clamped to the ~20s that exists). `--playback-selftest` + `--devices-selftest` regressions green. Built under the owner's standing "build the next thing" directive (lane selected 2026-07-31). **Remaining:** mapping an arbitrary live tile→`camera_id` (the live media↔inventory binding, a later media gate — this pass replays the `--rec-camera`), recorder/NVR/cloud instant retrieval, and RBAC/evidence gating |
| 24 (P3-15/P3-01) | **Spatial camera canvas** — the prototype's `spatial-canvas.js`, rebuilt natively "same or better" (owner directive 2026-07-31). A **Grid \| Spatial** toggle on the Live tab turns the wall into a pannable, zoomable world: wheel zooms toward the cursor (0.15–6×), dragging empty space pans, **dragging a tile repositions it** (P3-01's drag verb — the prototype could only pan), a click focuses. A pure `SpatialPolicy.h` core ports the prototype policy **exactly** — zones focus(≤24%)/peripheral(≤55%)/prewarm(240 px)/culled, zoom levels site(<0.4)/wing(<0.9)/room with tier caps, downgrade-now/promote-after-300 ms-settle — and `WorkspaceController::updateSpatialViewport` drives the **real governor** from it in one batched re-plan: culled tiles become `visible=false` → **PausedOffscreen, genuinely not decoding** (the prototype only skipped drawing), and budgets are the honest decode/mem/bw admission instead of the prototype's crude stream cap. Zoomed to site level tiles render as map-pin dots; tier labels are the prototype's Idle/Preview/Live·SD/Live·HD next to the honest state. Schema **v9** persists dragged positions (`workspace_tile.pos_x/pos_y`, −1=unset→grid default). `--spatial` starts in spatial mode; `--spatial-selftest`. Scope: [`spatial-canvas-P3-15-proposal.md`](spatial-canvas-P3-15-proposal.md) | ✅ built & verified (dev box): `--spatial-selftest` green — **33 checks** (prototype-exact zoom levels/zones/tier caps incl. focused-bypasses-cap, dwell anti-flap both ways, default grid placement, drag-never-replans, ONE batched re-plan per viewport pass, unchanged-pass no re-plan, culled→honest paused-offscreen, site-zoom immediate downgrade + focused survives, schema v9 + dragged-position save/restore round-trip). `persist_selfcheck` extended (v9 + position round-trip); all 9 CTest suites green; playback/instant/devices regressions green (75 devices checks); offscreen `--spatial --smoke-ms` clean (no QML errors/warnings) **and a real v8 DB migrated → v9 cleanly** in the smoke. **Remaining:** live video under the spatial tiles (the d3d11 compositor is fixed-geometry — later media slice, as the prototype likewise drew snapshots), floor-plan underlays/FOV cones + the premises entity (rest of P3-15), saved/named layouts (rest of P3-01) |
| 25 (P1-12/A0) | **Unified command envelope** — the first buildable slice of the A0 mandate. A pure, Qt-free `CommandRegistry` (`src/command/`, lib `vms_command`): every action is a `CommandSpec` (stable id, typed params with ranges/enums, **capability**, `dangerous` flag) and every invocation flows through **one gate** — validate (typed refusals: unknown-command / missing-param / bad-param / capability-denied / needs-confirm, nothing executes on a mismatch) → capability membership (Administrator grants all; role separation later changes the granted set, not the code) → **dangerous-action confirm** → execute → deterministic result — with an **audit record for every attempt, refusals included** (durable audit = P1-06). A deterministic **text leg** (`"focus 5"`, `"quality 3 thumb"`, trailing `confirm` = consent) and a **machine-readable JSON catalog** (P1-13's discovery seed, printed by `--commands`). `CommandController` (`src/workspace/`) registers the native verbs over the live controllers — `workspace.focus/quality/priority/layout/sweep/spatial`, `replay.start/live` (honest refusal without a rec DB), `device.rename/detach` + **`device.remove` (dangerous)** — and a **⌘ Command palette** (Ctrl+K) in the workspace types commands and shows the result + session audit trail. Scope: [`command-envelope-P1-12-proposal.md`](command-envelope-P1-12-proposal.md). `vms_cmdtest`/CTest + `--command-selftest` | ✅ built & verified (dev box): `command_selfcheck` green — **32 checks** (typed refusals never execute, extra/undeclared params refused, capability gate, dangerous needs-confirm→confirm, failing-handler result mapping, text leg incl. shorthand + trailing confirm, audit for refused AND executed, catalog with enum domains/ranges/dangerous marks, stable order). `--command-selftest` green — **18 checks** (text lines drive the REAL workspace: focus/quality/priority/layout/sweep; honest failures; needs-confirm; capability-denied; `spatial on` round-trips via signal; audit newest-first). All **10** CTest suites green; spatial/instant/playback/devices regressions green; offscreen smoke clean. **Remaining (own gates):** the external control API + coverage check (P1-13), voice leg (P1-12), durable audit (P1-06), real RBAC (P1-02/03), and rewiring every existing QML click through the envelope (P1-13 acceptance) |
| 26 (P1-13/A0) | **External control API** — the transport in front of the increment-25 envelope, so an external AI agent (or any client) drives the software with the **same authority model as a human operator**. `CommandServer` (`src/workspace/`, Qt HttpServer): **loopback-only** (binds 127.0.0.1, never public), **off by default** (`--api-port N`), **bearer-token** auth on every route (a random per-session token printed once; `--api-token` for scripted runs). `GET /v1/commands` = the machine-readable catalog (agent discovery); `POST /v1/invoke` `{"command","args","confirm"}` flows through the SAME registry gate — validation, capability, dangerous confirm, audit — with a deterministic status mapping (ok→200, unknown-command→404, missing/bad-param→400, capability-denied→403, **needs-confirm→409**, failed→500) and the same `{outcome,message}` body shape everywhere. Scope: [`command-api-P1-13-proposal.md`](command-api-P1-13-proposal.md). `--api-selftest` | ✅ first slice built & verified; increment 42 adds the bounded request-rate layer below. **Remaining (rest of P1-13):** multi-user/scoped tokens, TLS, result/event streaming, non-loopback exposure (security-reviewed), and the AI-agent connector (a client of this API). The UI-coverage check is complete. |
| 27 (P6-03/P6-07) | **Event rules & alarm lifecycle** — the operator's alarm surface. A pure Qt-free `AlarmEngine` (`src/events/`, lib `vms_events`): normalized events → enabled **rules** (event-type trigger + optional device filter, priority mapping, optional **auto-clear** event type) → deduplicated, **accountable alarms** (new → acknowledged (silenced, persists — re-ingest stays acknowledged) → escalated → cleared; a re-occurrence after clear is a **fresh** alarm); **maintenance suppresses notification, never the state**; honest `needsAttention`/`shouldNotify`; injectable time. **It notifies, never actuates** — automated linkage is the separately-gated P6-05; durable storage/backpressure is P6-04. First event source: `DeviceController` emits `healthTransition` (offline/degraded/recovered) from its live health feed — transitions only, steady state is silence — under three default rules (offline→High, degraded→Medium, recovery auto-clears). `AlarmController` + a **🔔 Alarms** toolbar chip (attention badge) + layered panel whose Ack/Esc/Clear buttons route **through the command envelope** (`alarm.ack/escalate/clear` — palette + API get them free; the first UI surface natively routed through commands). Scope: [`events-alarms-P6-proposal.md`](events-alarms-P6-proposal.md). `vms_eventtest`/CTest + `--alarms-selftest` | ✅ built & verified (dev box): `event_selfcheck` green — **30 checks** (matching incl. device filter + disabled rules, dedup bump, ack-persists-across-re-ingest, escalate, manual + auto-clear, fresh-after-clear, born-suppressed during maintenance, honest refusals, newest-first listing). `--alarms-selftest` green — **17 checks** end-to-end (a fake-probe outage raises the High alarm through the REAL DeviceController signal path, steady state raises nothing, `alarm.ack`/`alarm.escalate` through the audited envelope, recovery auto-clears, fresh alarm + maintenance suppression, verbs in the catalog). All **11** CTest suites + 7 workspace selftests green; offscreen smoke clean. **Remaining (own gates):** P6-05 linkage actions, P6-04 durable pipeline + throughput profile, rule schedules/correlation/simulation (rest of P6-03), delivery channels/SLO (rest of P6-07), event sources beyond health |
| 28 (P1-06) | **Durable audit history** — the durability the envelope's session trail lacked. Schema **v10** `audit_log` (forward-only additive) + `AuditRepo` (`vms_persist`, Qt-free): **append-only by contract** (no update/delete API exists) with every row **hash-chained** to its predecessor — `row_hash = SHA-256(prev_hash ‖ length-prefixed fields)`, hash injected so the repo stays dependency-free — so editing or deleting any row **breaks `verifyChain` from that exact row** (tamper evidence). The envelope's audit sink also appends durably when the store is open: every attempt from every surface — **palette / api / ui source-attributed**, refusals included — lands one row in the same standalone SQLite store. `--audit-dump [N]` prints the newest rows + the chain verdict. Scope: [`audit-log-P1-06-proposal.md`](audit-log-P1-06-proposal.md). `--audit-selftest` | ✅ built & verified (dev box): `--audit-selftest` green — **18 checks** (executed + refused both land durably, newest-first, palette/api attribution, genesis + chain links, untouched chain verifies, rows survive reopen, a direct-UPDATE tamper is **detected at exactly the forged row**, appends keep working with the historical break still evident, null-hash typed misuse). **End-to-end:** the app was driven over the API then exited; `--audit-dump` read back both the executed command and the refused destructive attempt, `[api]`-attributed, chain **intact**. All 11 CTest suites + 8 workspace selftests green; offscreen smoke clean. **Remaining (rest of P1-06):** login/credential/export auditing (needs P1-03 operator auth), before/after config values, trusted external timestamps, retention policy, chain-head anchoring |
| 30 (P1-13/A0) | **Playback transport commands.** The last named discrete QML mutation gap is closed: `playback.play/pause/speed/step/seek` and `replay.play/pause/speed/seek` use the existing `playback.review` capability and flow through the same validation, result mapping, API discovery, and audit as every other command. Playback-tab and instant-replay buttons use the structured QML path; high-frequency drag frames stay responsive as a declared continuous exemption, while release commits the final audited seek. Scope: [`playback-command-P1-13-proposal.md`](playback-command-P1-13-proposal.md) | ✅ built & verified (dev box): `--command-selftest` drives the real Playback controller; `--coverage-check` reports **0 violations** (39 mutators / 7 continuous or read-only sites) and no deferred-slice exemption remains. **Remaining:** voice, scoped tokens/TLS, streaming, roles/auth, and the AI-agent connector; rate limiting is complete in increment 42. |
| 31 (P3-15/A0) | **Premises + floor-plan/FOV spatial slice.** Schema **v11** adds canonical `premises_site`, `premises_floor`, and per-workspace active selection plus persisted tile `facing_deg`/`fov_deg`. Qt-free `PremisesRepo` + read-only QML `PremisesController`; audited `premises.site`, `premises.floor`, and `workspace.orientation` commands. The spatial canvas now shows an optional plan-image URI (or an honest named blank-floor fallback), active site/floor/timezone, and coverage wedges; none of this changes viewport/governor admission. Scope: [`premises-spatial-P3-15-proposal.md`](premises-spatial-P3-15-proposal.md) | ✅ built & verified (dev box): native build; schema v11 idempotence/reopen, default/custom premises, workspace position + orientation round-trips; command and spatial selftests; coverage **0 violations**; offscreen schema-v11 spatial smoke clean; root regressions green. Aggregate CTest 10/11 with the unchanged intermittent ONVIF Windows loader `0xc0000135`; focused ONVIF passes. **Remaining P3-15:** measured spatial performance and live-video-vs-map-tile decision; multi-site editor/navigation and P3-18 are outside this slice. |
| 32 (P3-15) | **Measured spatial performance gate.** `--spatial-benchmark [N]` measures the real 64-tile viewport classification → governor → QVariant model/signals path; `scripts/spatial-performance.ps1` supplies desktop runtime paths and preserves its non-zero result for automation. Idle and deliberately forced-replan scenarios emit CSV + human results and enforce 0.50/4.00 ms p95 budgets. Culled cameras skip FOV canvas painting. Scope: [`spatial-performance-P3-15-proposal.md`](spatial-performance-P3-15-proposal.md) | ✅ built & verified (dev box): Debug idle/active p95 **0.0026/1.9852 ms** with 0/2,000 replans; Release **0.0006/0.1995 ms** with 0/5,000 replans. Release 64-tile QML loaded in **105 ms** and offscreen smoke was clean. All 11 workspace gates and root regressions pass; aggregate native CTest remains 10/11 only because the unchanged ONVIF loader flake passes focused. Measurements and caveats: [`MEASUREMENTS.md`](MEASUREMENTS.md). **Remaining P3-15:** decide and build map pins versus live video on the spatial canvas. |
| 29 (P1-13/P1-09) | **Command coverage — no UI action bypasses the envelope.** Closes P1-13's enforcement half. `CommandController::invoke(id, argsMap, confirm)` gives QML a **structured** path through the same gate (typed args: names with spaces, URLs, credentials — no text tokenization); 12 new commands cover the rest of the discrete surface (`device.onboard/onboardRecorder/onboardDiscovered/discover/rescan/acknowledge/maintenance`, `channel.rename/disable/remove` (dangerous)`/move`, `workspace.place`); and **every one of the 27 direct controller call sites in the shipped QML is rerouted** — with *no* direct fallback, so a bypass cannot hide. The gate itself is a pure `CoverageCheck` that scans the **shipped** QML out of the Qt resource (what actually ships, not a stale source path), classifies each call against a declared 39-mutator table, and exits **non-zero** on a violation (`--coverage-check`, CI-gateable). Exemptions are declared, categorized and **printed** — pure reads and continuous viewport/scrub frames whose committed gestures are commands. **Security fix found by this work (P1-09):** an end-to-end run showed `password=s3cret` landing in the durable audit; `ParamSpec::secret` + a name-based backstop now redact secrets to `***` in every audit row, log line, and the catalog, while the handler still receives the real credential. Scope: [`command-coverage-P1-13-proposal.md`](command-coverage-P1-13-proposal.md) | ✅ built & verified (dev box): `--coverage-check` **0 violations** across 2 shipped QML files / 39 declared mutators; `--coverage-selftest` **10 checks** incl. the negative test; `command_selfcheck` **37 checks**. Leak closed end-to-end. All 11 component tests + 10 workspace gates green; smoke clean. **Remaining after inc 29:** voice, transport hardening, roles (Playback transport closed by inc 30) |
...
| 33 (P3-15) | **Adaptive spatial live tiles.** Site zoom stays pin-only, wing zoom stays honest state-card-only, and room zoom renders live imagery only for `live`/`degraded` tiles. Each spatial `VideoItem` subscribes to and crops the already-produced governed grid composite, so `GridPipeline` remains the single camera-session/decode/compositor owner; paused/offscreen/capacity tiles allocate no consumer and cannot show a stale crop. State/tier/focus/priority/FOV chrome remains above video. Scope: [`spatial-live-tiles-P3-15-proposal.md`](spatial-live-tiles-P3-15-proposal.md) | ✅ built & verified (dev box): Release 64-tile low-end offscreen smoke reported **16 eligible = 16 consumers = 16 receiving**, 223 delivered frames, and 16 unique crop ids; the other 48 delegates had no video consumer. Uneven composite crop mapping is pinned in `--spatial-selftest`. Release QML load **85 ms**; repeated controller benchmark p95 **0.0006 ms idle / 0.1874 ms active**. All 11 workspace gates and root regressions pass; focused ONVIF passes despite its unchanged aggregate-loader flake. **P3-15's approved standalone product scope is complete**; real-camera and physical low-end/display measurements remain hardware evidence. |

| 34 (P3-04) | **Honest live diagnostics.** `GridPipeline` observes each governed branch's decoded-buffer activity for playing/connecting/stalled/paused state and measures displayed FPS at the compositor→Qt boundary. `WorkspaceController.mediaDiagnostics` is a separate tile-id-indexed read model, keeping telemetry out of the governor hot path. The selected-camera overlay shows stream state, active codec/resolution/FPS, and explicit unavailable reasons for bitrate/latency/packet loss. Scope: [`live-diagnostics-P3-04-proposal.md`](live-diagnostics-P3-04-proposal.md) | ✅ first slice built & verified (dev box): Release 64-tile low-end smoke reported **16 active = 16 playing = 16 FPS/media-known**, **48/48 paused honest**, **29.7 FPS**, and 92 ms QML load. `--diagnostics-selftest` green; all 12 workspace gates/root regressions pass; Release spatial p95 remains 0.0006/0.1691 ms. Real-camera transport telemetry remains explicitly unavailable until the workspace has a source that exposes it. |

| 35 (P3-18) | **Premises operations panel foundation.** `SiteOperationsController` composes the active persisted site/floor/timezone and local clock with source-accurate device health and governed Live media diagnostics. A toggleable read-only panel sits on the workspace front layer; operating hours/open status, uptime/last-seen, cumulative duration, and analytics are explicit unavailable values with reasons rather than inferred data. Scope: [`premises-operations-P3-18-proposal.md`](premises-operations-P3-18-proposal.md) | ✅ first slice built & verified (dev box): `--site-operations-selftest` passed 15/15. Release persistent video smoke loaded QML in **103 ms** and reported the panel loaded with 6 source-accurate devices, 4 playing streams, and 24.6 FPS. All 13 workspace gates/root regressions pass; Release spatial p95 remains 0.0010/0.2185 ms. Schedule, uptime/last-seen, duration, and analytics sources remain later approved slices. |

| 36 (P3-18) | **Operating hours and open state.** Schema v12 persists a per-site schedule marker, multiple non-overlapping weekly windows, and closed/special-hours local-date exceptions. Five `premises.hours.*` commands use the audited envelope. Evaluation converts UTC through the persisted IANA timezone, including explicit spring-forward/repeated-hour behavior; the read-only panel shows Open/Closed, today's hours, and the governing rule. Scope: [`operating-hours-P3-18-proposal.md`](operating-hours-P3-18-proposal.md) | ✅ second slice built & verified (dev box): persistence/reopen/atomic-overlap checks pass; `--site-operations-selftest` passed **23/23** including DST edges. An authenticated API mutation reached the live panel. Release persistent video smoke loaded QML in **97 ms** with `hours=Open`, panel loaded, 4 playing streams, and 24.0 FPS. All 13 workspace gates/root regressions pass; Release spatial p95 remains 0.0006/0.1680 ms. |

| 37 (P3-18) | **Device-site ownership and reachability time.** Schema v13 gives devices optional canonical premises ownership and stores monotonic positive/negative reach observations, last positive seen, and current positive-run start. `device.site` uses the audited envelope. Restarted models expose historical last seen but require fresh session evidence before claiming current uptime; the premises panel excludes other-site/unassigned inventory. Scope: [`device-site-uptime-P3-18-proposal.md`](device-site-uptime-P3-18-proposal.md) | ✅ third slice built & verified (dev box): persistence **49/49**, Devices **85/85**, site operations **26/26**. Release persistent video smoke loaded QML in **92 ms** with schema v13, 6 assigned devices, 4 current reachability runs, latest seen, 4 playing streams, and 26.4 FPS. All 13 workspace gates/root regressions pass; Release spatial p95 remains 0.0006/0.1322 ms. |

| 38 (P3-18) | **Cumulative completed-recording duration.** `SegmentIndex::recordedDuration` validates canonical UTC, unions overlapping completed intervals per camera, and sums camera-time without duplicate inflation. The premises aggregate supplies only stable channels currently owned by the active site; orphan/unassigned/other-site footage is excluded. The panel shows duration and evidence while persistent streaming time remains explicitly unavailable. Scope: [`recording-duration-P3-18-proposal.md`](recording-duration-P3-18-proposal.md) | ✅ fourth slice built & verified (dev box): recording duration/validation gates green; site operations **29/29**. Release synthetic record→index→panel smoke showed **5 recorded seconds / 2 completed segments**, 91 ms QML load, 4 playing streams, and 26.1 FPS. All 13 workspace gates/root regressions pass; Release spatial p95 remains 0.0006/0.1120 ms. |

| 39 (P3-18) | **Persistent decoded streaming duration.** Schema v14 stores premises-scoped observed `playing` branch-time in milliseconds. `StreamDurationRepo` atomically adds monotonic elapsed × playing branches with overflow refusal; site switches settle the previous site and restarts never fill process downtime. The panel keeps recorded and streamed evidence separate and shows a combined camera-time only when both sources are available. Scope: [`streaming-duration-P3-18-proposal.md`](streaming-duration-P3-18-proposal.md) | ✅ fifth slice built & verified (dev box): persistence **54/54**, site operations **30/30**. Two Release persistent video runs advanced **18,708→29,248 ms** across reopen; the second loaded QML in **93 ms** with 4 playing branches at 25.6 FPS. All 13 workspace gates/root regressions pass; Release spatial p95 remains 0.0006/0.1377 ms; focused ONVIF passes despite the unchanged aggregate-loader flake. |

| 40 (P3-18) | **Site-scoped active alarm analysis.** The premises panel joins the native alarm engine's current active device-health alarms to current device→site ownership and reports attention, severity, acknowledgement, suppression, occurrences, and affected devices. The surface is explicitly session-only and excludes VCA and retained-history claims. Scope: [`alarm-analysis-P3-18-proposal.md`](alarm-analysis-P3-18-proposal.md) | ✅ sixth slice built & verified (dev box): site operations **32/32** including missing-source honesty, unassigned/site-switch exclusion, and lifecycle refresh; Release offscreen QML smoke loaded in **160 ms** with a valid zero-alarm state. All 13 workspace gates/root regressions pass; Release spatial p95 remains 0.0009/0.1717 ms; focused ONVIF passes despite the unchanged aggregate-loader flake. **P3-18's approved standalone scope is complete.** |

| 41 (P3-14) | **Continuity-preserving same-layout live re-plan.** The grid briefly pauses while `VideoItem` retains the last complete frame, replaces only tier-changed tiles with fresh decode/queue/upload/compositor-pad branches, preserves unchanged decoders and pads, then resumes. A full NULL rebuild is an explicit counted fallback; layout geometry changes keep their existing full rebuild. Scope: [`seamless-live-replan-P3-14-proposal.md`](seamless-live-replan-P3-14-proposal.md) | ✅ built & verified (dev box): Release 4/16/64-tile H.265/D3D12 smokes pass. The 64-tile stress completed **11 applies / 23 branch replacements / 0 full fallbacks**; subsequent applies took 25.2–70.8 ms, with 16 playing at 29.2 FPS and 46/46 paused branches honest. All 13 workspace gates/root regressions pass; Release p95 is 0.0006/0.1265 ms; focused ONVIF passes despite the unchanged aggregate-loader flake. |
| 42 (P1-13/A0) | **External control API rate limiting.** `CommandServer` now applies one fixed-window budget across catalog, invoke, unauthorized, malformed, and unknown-route traffic. Default/configurable `--api-rate-limit 120` per 60 seconds; excess requests receive structured HTTP 429 before the envelope can execute a command. Scope: [`api-rate-limit-P1-13-proposal.md`](api-rate-limit-P1-13-proposal.md) | ✅ built & verified (dev box): `--api-selftest` **21/21** including 2 admitted requests, third rejected, and unchanged workspace state; non-positive CLI budget exits 2. All 13 workspace gates + root regressions pass; aggregate CTest 10/11 only on the known ONVIF loader flake and focused ONVIF passes; Release spatial p95 0.0006/0.1136 ms. |
| 43 (P1-12/A0) | **Deterministic recognized-speech invocation.** A pure C++ `VoiceCommandMapper` accepts a transcript, registered BCP-47 grammar tag, and confidence from an interchangeable recognizer, maps only finite phrases to typed command arguments, and invokes the existing envelope with source `voice`. Unsupported language, low confidence, ambiguity, and no-match fail closed; voice can never provide destructive confirmation. Scope: [`voice-command-P1-12-proposal.md`](voice-command-P1-12-proposal.md) | ✅ built & verified (dev box): `command_selfcheck` **50 checks** and `--command-selftest` **40 checks**; real workspace mutation, side-effect-free refusals, audit source attribution, and spoken-confirm isolation pass. All 13 workspace gates + root regressions pass; aggregate CTest 10/11 only on the known intermittent ONVIF loader failure and focused ONVIF passes. **P1-12 is complete.** |
| 44 (P1-06) | **Structured audit + trusted-anchor boundary.** Schema v15 retains legacy v1 chain verification and adds canonical v2 category/actor/subject/correlation plus redacted before/after state. `AuditRepo` exports an exact SHA-256 chain-head request and persists an RFC 3161 receipt only after an injected cryptographic verifier accepts it and the live head still matches. Scope: [`audit-structure-anchor-P1-06-proposal.md`](audit-structure-anchor-P1-06-proposal.md) | ✅ second slice built & verified (dev box): `vms_dbtest` **54/54**, `--audit-selftest` **28/28** including stale-head rejection, all 13 workspace gates and root regressions pass; aggregate CTest 10/11 only on the known intermittent ONVIF loader failure and focused ONVIF passes. Production TSA policy, remaining event producers, and anchor-safe retention remain. |

| 45 (P6-03/P6-07) | **Advanced rule and alarm-delivery core.** Monotonic rule versions, UTC ISO-weekday/overnight schedules, conditions, same-device correlation/dependencies, side-effect-free simulation, measured counters, assignment, SLA/SLO breaches, stable idempotent connector jobs, bounded exponential retry, maintenance suppression, and lifecycle audit callbacks. `alarm.assign` uses A0. Scope: [`alarm-rules-delivery-P6-proposal.md`](alarm-rules-delivery-P6-proposal.md). | Built and verified: functional gates plus a 1,000-rule/1,000-event profile (1,000,000 evaluations, approximately 1.04 microseconds/rule on this dev run); `--alarms-selftest` verifies real-model assignment through the envelope. Provider-specific SMTP/SMS/webhook delivery remains unclaimed pending selection and credentials. |
| 46 (P1-13/P1-06) | **Scoped external-control identities.** Each bearer identity has a stable actor ID and least-privilege capability set; discovery is filtered and invocation uses those grants. Results and structured audit carry safe/generated correlation IDs. Empty/duplicate identities fail closed and unknown routes now authenticate. Production flags select identity and scopes. Scope: [`api-scoped-identities-P1-13-proposal.md`](api-scoped-identities-P1-13-proposal.md). | Built and verified over real loopback HTTP: filtered catalog, side-effect-free 403, actor/correlation attribution, existing confirmation/status/rate gates. Remote TLS remains disabled pending deployment hostname/certificate/trust/IdP inputs. |

| 47 (P2-14/P3-04) | **Deterministic RTSP transport + ONVIF test-runtime hardening.** A typed policy selects `auto`, TCP interleaved, UDP unicast, or UDP multicast lower transport, enables RTCP, and applies bounded UDP fallback/TCP timeouts only to compatible GStreamer sources. CTest supplies the Qt runtime path and bounds ONVIF execution. Scope: [`connectivity-protocols-P2-14-proposal.md`](connectivity-protocols-P2-14-proposal.md). | Built and verified: RTSP policy **13/13**, focused ONVIF **50/50**, diagnostic pipelines build, and aggregate native CTest **12/12**. Ports such as 8000/34580 remain vendor service/SDK concerns, not evidence of RTSP media transport. |
| 48 (P2-14/P3-03/P3-04/P3-14) | **Broker-backed production workspace live sources.** Repeatable `--live-camera ID` binds exact inventory identities to tiles; the governor leases Main or Sub through `ConnectionBroker`, Paused leases nothing, and `--rtsp-transport` applies the explicit lower-transport policy. Failed sources are torn down, their broker session is released with typed feedback, and only that tile becomes black/unavailable. Credential-bearing URIs are scrubbed and never logged. Scope: [`workspace-live-sources-P2-P3-proposal.md`](workspace-live-sources-P2-P3-proposal.md). | Built and verified: broker/policy focused tests pass; missing-camera live smoke reports **1/1 unavailable** and exits 0 without a URI leak; synthetic smoke remains **16/16 playing at 27.1 FPS**; all 13 workspace gates, full native build, aggregate native CTest **12/12** including ONVIF, and root regressions pass. Valid-credential device/NVR and low-end-machine evidence remain. |
...
# inc 23) Instant playback from live (P3-05) — the "instant replay" verb. On the
#   Live tab, select a camera and click "Instant replay · 10s/30s/60s" in the
#   info panel: a replay overlay opens over the (still-running) live wall showing
#   the last N seconds of the recording-backed camera with a transport + a
#   "⟵ Live" return. Needs a recording DB (from vms_record) via --rec-db/--rec-
#   camera; honestly shows "No local recording for this camera" when absent.
$env:PATH = "C:\Qt\6.11.1\msvc2022_64\bin;C:\Program Files\gstreamer\1.0\msvc_x86_64\bin;$env:PATH"
$env:QT_PLUGIN_PATH = "C:\Qt\6.11.1\msvc2022_64\plugins"
$env:GST_PLUGIN_PATH = "C:\Program Files\gstreamer\1.0\msvc_x86_64\lib\gstreamer-1.0"
.\build\vms_workspace.exe --video --rec-db rec.db --rec-camera cam-1
# Headless verification (no display, no camera):
.\build\vms_workspace.exe --instant-selftest            # 22-check controller logic
$env:QT_QPA_PLATFORM = "offscreen"                       # QML bindings + real decode
.\build\vms_workspace.exe --smoke-ms 4000 --rec-db rec.db --rec-camera cam-1 --no-optimize

# inc 24) Spatial camera canvas (P3-15/P3-01) — the prototype's spatial grid,
#   native. On the Live tab click "⌖ Spatial" (or start with --spatial): the
#   wall becomes a pannable/zoomable canvas — wheel zooms toward the cursor,
#   drag empty space to pan, DRAG A TILE to place it (persisted across
#   restarts), click to focus, "⤢ Fit" to frame everything. Zoom out to site
#   level and tiles become map-pin dots with everything honestly paused; zoom
#   in and center tiles earn Main while culled ones truly stop decoding.
$env:PATH = "C:\Qt\6.11.1\msvc2022_64\bin;C:\Program Files\gstreamer\1.0\msvc_x86_64\bin;$env:PATH"
$env:QT_PLUGIN_PATH = "C:\Qt\6.11.1\msvc2022_64\plugins"
$env:GST_PLUGIN_PATH = "C:\Program Files\gstreamer\1.0\msvc_x86_64\lib\gstreamer-1.0"
.\build\vms_workspace.exe --spatial --count 25
# Headless verification (no display, no camera):
.\build\vms_workspace.exe --spatial-selftest            # 33-check policy + governor logic
$env:QT_QPA_PLATFORM = "offscreen"                       # QML bindings, no window
.\build\vms_workspace.exe --spatial --smoke-ms 3000 --no-optimize --no-health-feed

# inc 25) Command palette (P1-12/A0) — every operator verb as one validated,
#   audited command. Click "⌘ Command" (or Ctrl+K) and type:
#     focus 5 · quality 3 thumb · priority 2 high · layout 16 · sweep off ·
#     spatial on · replay.start 30 · replay.live · device.rename <id> <name> ·
#     device.remove <id> confirm   (destructive commands refuse without confirm)
#   The palette shows the deterministic result and the session audit trail —
#   refused attempts included. Machine-readable catalog (the P1-13 seed):
.\build\vms_workspace.exe --commands
# Headless verification:
.\build\vms_cmdtest.exe                                  # 32-check pure envelope
.\build\vms_workspace.exe --command-selftest             # 18-check live wiring

# inc 26) External control API (P1-13/A0) — drive the running app from any
#   external client (the AI-agent path) through the SAME validated, audited
#   gate. Loopback-only, off by default; the bearer token prints at startup.
.\build\vms_workspace.exe --api-port 18790 --api-rate-limit 120
#   ...then, from any client:
#     GET  http://127.0.0.1:18790/v1/commands            (catalog discovery)
#     POST http://127.0.0.1:18790/v1/invoke              (Authorization: Bearer <token>)
#          {"command":"workspace.layout","args":{"count":9}}
#          {"command":"device.remove","args":{"id":"x"}}          -> 409 needs-confirm
#          {"command":"device.remove","args":{"id":"x"},"confirm":true}
# Headless verification:
.\build\vms_workspace.exe --api-selftest                 # 16-check HTTP round-trips

# inc 27) Alarms (P6-03/P6-07) — click "🔔 Alarms" in the toolbar. A device
#   going offline (live health feed) raises a High alarm that demands
#   attention until acknowledged; recovery auto-clears it. Ack/Esc/Clear run
#   through the command envelope (also: "alarm.ack 1" in the palette, or the
#   API). Maintenance (Devices tab) suppresses notification, never the alarm.
# Headless verification:
.\build\vms_eventtest.exe                                # 30-check pure engine
.\build\vms_workspace.exe --alarms-selftest              # 17-check end-to-end

# inc 28) Durable audit (P1-06) — every envelope attempt (palette/API/panel,
#   refusals included) lands one hash-chained, append-only row in the store.
#   Read it back any time, even after the app exits; the chain verdict says
#   whether the log was tampered with:
.\build\vms_workspace.exe --audit-dump 20
# Headless verification:
.\build\vms_workspace.exe --audit-selftest               # 18-check durability + tamper detection

# inc 29) Command coverage (P1-13) — the gate that keeps the UI honest: every
#   state-changing click routes through the envelope, so nothing escapes the
#   capability check, the confirm gate, or the audit. Non-zero exit on a
#   bypass, so it belongs in CI:
.\build\vms_workspace.exe --coverage-check
# Headless verification (includes a negative test: an injected bypass MUST fail):
.\build\vms_workspace.exe --coverage-selftest            # 10-check gate

# inc 30) Playback transport commands (P1-13/A0) — the Playback tab and
# instant-replay overlay now drive play/pause/speed/frame-step/seek through the
# same validated, capability-checked, audited envelope as the palette and API:
#   playback.play · playback.pause · playback.speed 2 · playback.step -25 ·
#   playback.seek 0.5 · replay.play · replay.pause · replay.speed 4 · replay.seek 0
# Scrub motion stays responsive locally; releasing the drag commits one audited seek.
# Verification is included in --command-selftest and --coverage-selftest.
```
