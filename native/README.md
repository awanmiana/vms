...
full breaker open→refuse→cooldown→recover cycle via an injected clock). **6c-2** — rewiring
`vms_grid`/`vms_workspace` to connect through the broker by `camera_id` instead of a
URL-with-password — remains (build-verifiable here; live-camera decode is credential-blocked, the
same item already tracked for 4b).
