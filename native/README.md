...
| 9 (P1-10/P0-03/A0) | Node-connectivity resilience core — failover across **this software's own** peer / AI-agent endpoints (host:port, local/LAN/internet), **not cameras**. Pure Qt-free `NodeRegistry` (`src/node/`): P0-03 backoff/circuit-breaker health, `select()` returns the most-preferred usable endpoint and fails over when the primary trips, honest Unavailable when all down, injectable clock. `vms_nodetest`/CTest | ✅ built & verified (dev box): `node_selfcheck` green — 21 checks (preference ordering, failover local→LAN→internet, recovery on success + after cooldown, all-down honest Unavailable, honest health states). All 7 CTest suites green. **No live sockets** — the peer transport, discovery, auth, and AI-agent connector are P1-10 / A0; this is the decision core behind the interface they implement unchanged |
